...
class ApiRepository:
    ...
    def fetch_sent_paper_airplanes(
        ...
        """유저가 보낸 종이비행기를 가져옵니다."""
        df = pl.read_csv("store/paper_airplane.csv")
        data = df.filter(pl.col("sender_id") == sender_id).sort(
            "created_at", descending=True
        )
        count = len(data)
        paper_airplanes = data.slice(offset, limit).to_dicts()
        return count, [
            models.PaperAirplane(**paper_airplane) for paper_airplane in paper_airplanes
        ]
